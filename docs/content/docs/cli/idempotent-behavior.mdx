...
description: Running lunar-css init twice is always safe — it never overwrites your edits.
---
...
Checked via substring: if the file already contains `'lunar-css/metro'`, the CLI skips it. Otherwise it performs an [AST merge](/docs/cli/ast-merge).
...
Checked for the `# LunarCSS` sentinel header. If the `.lunar-css/` entry is already present, no changes are made.

### `tsconfig.json`

Checks whether `"lunar-css/types"` is already in `compilerOptions.types`. If present, skipped.
