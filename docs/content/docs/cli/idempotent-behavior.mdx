...
Checked via substring: if the file already contains `'@lunar-kit/css/metro'`, the CLI skips it. Otherwise it performs an [AST merge](/docs/cli/ast-merge).
...
Checked for the `# LunarCSS` sentinel header. If the `.@lunar-kit/css/` entry is already present, no changes are made.
