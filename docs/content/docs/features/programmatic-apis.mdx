...
All three APIs read the same global registry. The web bundle's runtime hydrates the registry from `lunar-css/__theme__` exactly like native, so `vars.primary` returns `"#6366f1"` on both platforms — no Tailwind dependency, no DOM-side custom-property lookup.
