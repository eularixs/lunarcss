...
`lunarcss init` does this automatically.
...
After adding `"lunarcss/types"`, TypeScript accepts `className` on all these components:
...
Without `"lunarcss/types"` in `tsconfig.json`, TypeScript will error:
...
```ts
import type { LunarConfig } from 'lunarcss'
...
```ts
import type { ThemeExtend } from 'lunarcss'
