...
3. **Routes `lunarcss/__theme__`** — adds a `resolveRequest` interceptor that redirects the bare specifier to the generated file
...
`src/index.ts` imports `THEME_TOKENS` from `'lunarcss/__theme__'`:
...
On native:
- Metro's `resolveRequest` intercepts `'lunarcss/__theme__'`
...
`withLunarCSS` sets `babelTransformerPath` to `lunarcss/metro/transformer`. The transformer:
...
5. Injects `import { __lcssTw } from 'lunarcss/runtime'` if not already present
...
`lunarcss init` adds this automatically.
