...
2. **Emits `.@lunar-kit/css/__theme__.js`** — a frozen JS object of flattened token key→value pairs. Content-hash delta checked — only rewritten when tokens change
...
- Routes to `.@lunar-kit/css/__theme__.js` in the project root
- `.@lunar-kit/css/__theme__.js` contains `export const THEME_TOKENS = Object.freeze({ '--color-primary': '#6366f1', ... })`
...
## `.@lunar-kit/css/` directory

LunarCSS writes generated files to `.@lunar-kit/css/` at the project root. Add it to `.gitignore`:
